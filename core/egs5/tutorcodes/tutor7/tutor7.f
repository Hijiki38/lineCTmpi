...
! on it with or without fluorescence photons and Doppler broadening.
!
...
! Use log10(ncases) as maximum value
      ...
      if (iedgfl(2).eq.1) then
        write(6,160) ein,pcoord(3,2),ncases
160     format(/' Reflected photon spectrum'/' for a',F8.2,
     ...
     *  25X,' log(counts for ',I8,' incident photons)')
      else
        write(6,170) ein,pcoord(3,2),ncases
...
!  Reblank
      ...
      write(6,190)incohr(2),iprofr(2)
190   format(' incohr(2)=',i2,' iprofr(2)=',i2)
      stop
      end
