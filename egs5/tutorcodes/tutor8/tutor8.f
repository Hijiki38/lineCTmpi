...
!                     **************                                    
!                                                                       
! An EGS5 user code which scores the spectrum of photons reflected from
! a 1.0 cm thick slab of lead when a 100 keV beam of electrons is 
! incident on it, with or without fluorescence photons from electron
! impact ionization and/or photo-ionization of bremsstrahlung photons.
! Particle splitting is demonstrated.
!
!  For SLAC-R-730/KEK Report 2005-8:  Example of splitting
!
...
! there is particle transport - just region 2 in this case
      incohr(2)=0
!     1: Turn on incoherent scattering function in the slab
!     0: Turn off incoherent scattering function in the slab
      iprofr(2)=0
!     1: Turn on Doppler broadening in the slab
!     0: Turn off Doppler broadening in the slab
      impacr(2)=1
!     1: Turn on electron impact ionization in the slab
!     0: Turn off electron impact ionization in the slab
      ieispl=1
!     1: Turn on eii photon splitting
!     0: Turn off eii photon splitting
      neispl=20
!     neispl: number of splitting for eii photon
      ibrspl(1)=0
      ibrspl(2)=1
      ibrspl(3)=0
!     1: Turn on brems photon splitting
!     0: Turn off brems photon splitting
      nbrspl=30
!     nbrspl: number of splitting for brems photon
      ibrdst=1
!     1: Turn on brems angular distribution
!     0: Turn off brems angular distribution
...
!-----------------------------------------------------------------------
! Define initial variables for 100 keV beam of electrons normally incident
! on the slab
      iqin=-1
!     Incident electrons
!             100 keV
...
! defined before hatch call
      ...
      write(6,130)
130   format(/' Start tutor8'/' Call hatch to get cross-section data')
...
!-----------------------------------------------------------------------
! Use log10(ncases) as maximum value
      binmax=alog10(float(ncases))

      if (iedgfl(2).eq.1) then
        write(6,160) ein,pcoord(3,2)
160     format(/' Reflected photon spectrum'/' for a',F8.2,
     *  ' MeV pencil beam of electrons on a',F7.2,
     *  ' cm thick slab of lead'/' with fluorescence photon'//T6,
     *  'Energy  counts/incident electron'/
     *  25X,' log(counts for 10^6 incident electrons)')
      else
...
! Blank entire output array
      do j=1,50
        if(ebin(j).gt.0) then
          icol=
     *      int(dlog10(ebin(j))/binmax*48.0+0.999)
          if (icol.le.0) icol=1
        else
          icol = 1
        endif
...
!  Load output array at desired location
        write(6,180) bwidth*j,ebin(j)/float(ncases),line
180     format(F10.4,F12.6,48A1)
        line(icol)=' '
!  Reblank
      end do

      write(6,190)impacr(2),ieispl,neispl
190   format(' impacr(2)=',i2,' ieispl=',i2,' neispl=',i3)
      write(6,200)ibrspl(2),nbrspl,ibrdst
200   format(' ibrspl=',i2,' nbrspl=',i3,' ibrdst=',i2)
      stop
      end
...
!***********************************************************************
!
!  In this AUSGAB routine for TUTOR8 we score photons reflected
...
!***********************************************************************
      ...
      if(irl.eq.1.and.iq(np).eq.0) then   
! Photon is reflected (emitted backward relative to source)
!     Increment bin corresponding to photon energy
        ibin= min0 (int(e(np)/bwidth + 0.999), 50)
        if (ibin.ne.0) then
          ebin(ibin)=ebin(ibin)+wt(np)
        end if
      end if
...
!     ----------------------
      ...
      else            ! We are in the Pb slab - check the geometry
